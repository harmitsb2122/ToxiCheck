                ...
                await getResult(data).then((ans) => {
                  if (ans != 6) {
                    if (
                      ...
                      // get the newly created span element
                      const spanElement = currentElement.querySelector("span");
                      // set the cursor position after the span
                      ...
                      // add event listener to append text after the span
                      currentElement.addEventListener("keypress", function (e) {
                        if (e.key !== "Tab" && e.key !== "Shift") {
                          const textNode = document.createTextNode(e.key);
                          spanElement.after(textNode);

                          // set cursor position after text node
                          const endRange = document.createRange();
                          endRange.selectNodeContents(currentElement);
